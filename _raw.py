...
class ATC:
    def __init__(self) -> None:
        self.model = models.LoopNet(n).to(device)
        self.load_data()
    ...
    def load_data(self):
        self.dataset_s = datasets.ImageFolder(root=os.path.join('Office31', args.source, 'images'))
        self.dataset_t = datasets.ImageFolder(root=os.path.join('Office31', args.target, 'images'))
        dataset_t_test = datasets.ImageFolder(root=os.path.join('Office31', args.target, 'images'))

        self.dataset_s_n_index = [_ in [*range(n)] for _ in self.dataset_s.targets]
        self.dataset_s_samples = np.array(self.dataset_s.samples)[self.dataset_s_n_index]
        self.dataset_t_n_index = [_ in [*range(m)] for _ in self.dataset_t.targets]
        self.dataset_t_samples = np.array(self.dataset_t.samples)[self.dataset_t_n_index]
    ...
    def UpdateModel(self):
        if self.model.num_classes != len(self.nl):
            ...
            self.model.cls_fc = nn.Linear(256, len(self.nl))
    ...
    def DSAN_test(self, dataloader):
        ...
        with torch.no_grad():
            ...
            # print(
            #     f'Seen Accuracy:  {seen_correct}/{seen_total} ({100. *  seen_correct / seen_total:.2f}%) \t Total Accuracy: {seen_correct}/{len(dataloader.dataset)} ({100. * seen_correct / len(dataloader.dataset):.2f}%)')
        return seen_correct / seen_total, seen_correct, seen_correct / len(dataloader.dataset)
    ...
    def DSANTrain(self, nepoch = 10):
        ...
        self.optimizer = torch.optim.SGD([
            ...
            {'params': self.model.bottle.parameters(), 'lr': 0.01},
        ...
        with trange(1, self.nepoch + 1) as pbar:
            for epoch in pbar:
                ...
                self.optimizer.param_groups[2]['lr'] = 0.01 / math.pow((1 + 10 * (epoch - 1) / self.nepoch), 0.75)
                self.DSAN_train(epoch, self.model, self.source_loader, self.target_loader_disorder, self.optimizer, pbar)

                t_seenAcy, t_correct, t_AllAcy = self.DSAN_test(self.target_loader_disorder)
                if t_correct >= correct:
                    correct = t_correct
                    stop = 0
                    torch.save(self.model.state_dict(), 'model.pth')

                seen_correct = max(seen_correct, t_seenAcy)
                # print(f'MAX Seen accuracy: {100. * seen_correct:.2f} \t All accuracy: {100. * correct / len(self.target_dataset):.2f}%\n')
                pbar.set_postfix(MAXSeenAccuracy=100. * seen_correct, AllAccuracy=100. * correct / len(self.target_dataset))
        
        self.model.load_state_dict(torch.load('model.pth'))
        self.progress[-1] = [len(A.nl), len(list(A.Ssd.values()))/len(self.target_dataset), seen_correct, correct / len(self.target_dataset)]
