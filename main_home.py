...
for clsname in class_name_alllist[:n]:
  shutil.copytree(os.path.join(args.root_path, 'OfficeHome', args.source, clsname), os.path.join('Semisup', args.source, clsname))

for clsname in (class_name_alllist[:n]+class_name_alllist[-m:]):
  shutil.copytree(os.path.join(args.root_path, 'OfficeHome', args.target, clsname), os.path.join('Semisup', args.target, clsname))
...
class ATC:
    ...
    def load_data(self):
        ...
        self.dataset_t = datasets.ImageFolder(root=os.path.join('Semisup', args.target), transform=train_transform)
        ...
        self.source_loader = torch.utils.data.DataLoader(self.source_dataset, batch_size=batch_size, shuffle=True, kwargs)
        ...
        self.target_loader_disorder = torch.utils.data.DataLoader(self.target_dataset, batch_size=batch_size, shuffle=True, kwargs)
        ...
        # print('self.source_dataset.samples:',self.source_dataset.samples)
    ...
    def UpdateSource(self):
        ...
        # print('self.source_dataset.label:',self.source_dataset.label)
        ...
        self.NEXsource_loader = torch.utils.data.DataLoader(self.source_dataset, batch_size=batch_size, shuffle=True, drop_last=True, kwargs)
        ...
        print(f'Number: {len(self.source_dataset)}\t','Target Domain Category:',set(self.source_dataset.label))
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if A.uncertaintySample(Visio = False):
            break
        if A.clustersSample(Visio=False):
            break
        A.UpdateSource()
        A.UpdateModel()
        A.DSANTrain(nepoch = 50)
