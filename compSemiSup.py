...
parser.add_argument('--c_n', type=int, default=4, help='Number of cluster classes')
parser.add_argument('--s_n', type=int, default=5, help='Number of samples per unknown cluster')
parser.add_argument('--m_n', type=int, default=4, help='Minimum number of training')
...
class ATC:
    ...
    def clustersSample(self, max_epochs = 10, Visio = False):
        ...
        cosine_clusters = CosineClusters(args.c_n, Euclidean=False)
        ...
        for i in self.ul:
            ...
            k = [int(args.s_n/j)]*(j-1) + [args.s_n - int(args.s_n/j)*(j-1)]  # 从每个簇中取多少数据   
        ...
        for index, _label in enumerate(set(list(self.Sub.values())) | set(self.ul)):
            ...
            if _label not in self.SubLabel or self.SubCount[self.SubLabel.index(_label)] < args.m_n and _label not in self.ul:
                continue
    ...
    def DSAN_test(self, dataloader):
        ...
        if unkown_total == 0:
            # 全局粗略精度OS、已知类别精度、已知类别正确个数、已知类别全局精度OSF、共享精度OS'、细腻未知精度、粗略未知精度、未知细腻预测正确列表、未知细腻预测错误列表
            return (share_correct+unkown_cover_correct) / (share_total+unkown_total), \
                    seen_correct / seen_total, \
                    seen_correct, \
                    seen_correct / len(dataloader.dataset), \
                    share_correct / share_total, \
                    -1, \
                    unkown_cover_correct / unkown_total, \
                    unkown_correct_count, \
                    unkown_error_count
        else:
            return (share_correct+unkown_cover_correct) / (share_total+unkown_total),\
                    seen_correct / seen_total, \
    ...
    def DSANTrain(self, nepoch = 10):
        ...
        self.progress[-1] = [len(A.nl), f'query rate: {len(list(A.Ssd.values()))}/{len(self.target_dataset)}={len(list(A.Ssd.values()))/len(self.target_dataset)}', 
                            f'max seen acy: {max_seenAcy}',    # 可以充当 OSF
                            f'max cover acy: {max_CoverAcy}',  # OS
                            f'max share acy: {max_shareAcy}',  # OS'
                            f'max unkown acy: {max_unknoweAcy}', 
                            f'max unkown cover acy: {max_unknoweCoverAcy}', 
                            f'max all acy: {max_correct / len(self.target_dataset)}', 
                            best_c ,best_e]
